...
def roof_server_polling(server_address, server_port, status_file_path):
    while True:
        try:
            # Establish a connection to the roof server
            with socket.create_connection((server_address, server_port)) as sock:
                while True:
                    ...
                    if match:
                        ...
                        roof_status = 0 if stat == 0 else 1  # 0 if roof is open, 1 if roof is closed
                        update_status_file(status_file_path, roof_status)
                    else:
                        update_status_file(status_file_path, 2)  # Unexpected data format
        except Exception as e:
            print(f"Connection error: {e}")
            update_status_file(status_file_path, 2)  # Connection error
